...
const unsigned long period = 8000; // us  <-- the time each servo write is commanded 
unsigned long h = 0;
unsigned long j = 0;
...
void loop() {
  ...
  if(current - previous1 >= period) {
    ...
    */
    ...
    sBlue.detach(); sYellow.detach(); sBlue3.detach(); sYellow3.detach(); 
  }
  }

  // Timed Event 2 //
  if (current/1000 > (period/1000)*endp*2.6) {
    ...
    sBlue1.detach(); sYellow1.detach(); sBlue2.detach(); sYellow2.detach();
  }
  }
  }
 Serial.println(""); 
